...
const useInitialLayoutHandler = params => {
  ...
  const ancestorAnimations = ancestorKeys.map(key => AnimationStore.getAnimation(key, "animating"));
...
const useBlurMeasurement = params => {
  ...
  const ancestorClosing = [current.route.key, ...ancestorKeys].map(key => AnimationStore.getAnimation(key, "closing"));
...
const {
  ...
  const maybeMeasureAndStore = useStableCallbackValue(({
    ...
    emitUpdate();
    BoundStore.registerSnapshot(sharedBoundTag, currentScreenKey, correctedMeasured, preparedStyles, ancestorKeys);
    if (shouldSetSource) {
      if (isAnimating.get()) {
        ...
        if (existing) {
          BoundStore.setLinkSource(sharedBoundTag, currentScreenKey, existing.bounds, preparedStyles, ancestorKeys);
        }
        return;
      }
      BoundStore.setLinkSource(sharedBoundTag, currentScreenKey, correctedMeasured, preparedStyles, ancestorKeys);
    }

    // Set as destination (on mount during animation)
    if (shouldSetDestination) {
      BoundStore.setLinkDestination(sharedBoundTag, currentScreenKey, correctedMeasured, preparedStyles, ancestorKeys);
    }
