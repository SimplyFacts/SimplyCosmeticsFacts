...
export const useGestureViewer = ({
  ...
  useEffect(() => {
    if (!autoPlay || !manager || dataLength <= 1 || isZoomed || isRotated || !enableLoop && currentIndex === dataLength - 1) {
      return;
    }
  ...
  useEffect(() => {
    if (shouldStartTriggerAnimation && triggerRectRef.current) {
      ...
      const initialScaleFromTrigger = Math.min(triggerRectRef.current.width / width, triggerRectRef.current.height / height);
  ...
  }, [shouldStartTriggerAnimation, animationConfig, width, height, triggerOpacity, triggerScale, triggerTranslateX, triggerTranslateY, onAnimationComplete]);
  useEffect(() => {
  ...
  }, [dataLength, enableSwipeGesture, initialIndex, manager, width, itemSpacing, maxZoomScale, enableLoop, scale, height, translateX, translateY, rotation]);
  useEffect(() => {
  ...
  }, [animationConfig, onDismiss, onDismissStart, width, height, triggerTranslateX, triggerScale, triggerTranslateY, triggerOpacity]);
  ...
  const dismissGesture = useMemo(() => {
    ...
    return Gesture.Pan().minDistance(10).averageTouches(true).activeCursor('grabbing').activeOffsetY([-10, 10]).failOffsetX([-10, 10]).enabled(canDismiss).onUpdate(event => {
  ...
  }, [scale, enableZoomGesture, maxZoomScale, translateX, translateY, startScale, initialTranslateX, initialTranslateY, width, height, constrainTranslation]);
  ...
  const backdropStyle = useAnimatedStyle(() => {
    ...
    return {
      opacity: dismissOpacity * baseOpacity
    };
  }, [animateBackdrop]);
